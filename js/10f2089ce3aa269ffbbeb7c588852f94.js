...
{$(form).trigger('order_ajax_submit.before');this.onSubmitBefore(form,1);$(form).ajaxSubmit({success:function(result,statusText,xhr,form){diafan_ajax.result(form,result);$(document).trigger('order_ajax_submit.after');diafan_ajax.manager.onSubmitAfter(form,0);$(document).trigger('order_ajax_submit.after_last');diafan_ajax.manager.onSubmitAfterLast(form)}})}
...
return!1},manager:{order:[],count:function(){var count=0;this.order.forEach(function(item,index,list){count++});return count},flag_busy:!1,addForm:function(form){this.order.push(form);this.run();return this},onSubmitBefore:function(form,order_count){},onSubmitAfter:function(form,order_count){},onSubmitAfterLast:function(form){},run:function(){var count=this.count();if(!this.flag_busy&&count>0)
{this.flag_busy=!this.flag_busy;var form=this.order.shift();$(form).trigger('order_ajax_submit.before');this.onSubmitBefore(form,count);$(form).ajaxSubmit({async:!0,beforeSubmit:function(arr,form,options){return!0},success:function(result,statusText,xhr,form){diafan_ajax.result(form,result);var current_count=diafan_ajax.manager.count();$(document).trigger('order_ajax_submit.after');diafan_ajax.manager.onSubmitAfter(form,current_count);if(current_count<=0)
{$(document).trigger('order_ajax_submit.after_last');diafan_ajax.manager.onSubmitAfterLast(form)}},error:function(data){},complete:function(xhr,statusText){diafan_ajax.manager.flag_busy=!1;if(diafan_ajax.manager.count()>0)
...
else result=!0;return!1});return result}})(jQuery);var defer_loading={options:{container:window,emergence:{async:!0,},},_core:{flag_init:!1,defer_id:0,objects:function(){return $('form input[name="defer"]').filter(':not([loading=true])')},prepare_async:function(form){if(!form.length)return form;this.defer_id++;form.attr("defer_id",this.defer_id).append('<input type="hidden" name="ajax_async" value="1">').find('input').filter('[name^="attributes"]').each(function(){var name=$(this).attr("name").replace(/^attributes/gi,"attributes["+defer_loading._core.defer_id+"]");$(this).attr("name",name)});return form},after_submit:function(form){var objects=defer_loading._core.objects();if(!objects.length)
...
$(document).on('ready',function(e){$(document).on('order_ajax_submit.after_last',function(){var init_func='init_main';if(init_func&&typeof window[init_func]=='function')
{window[init_func]()}
